...
bool GLGizmoIroning::on_init()
{
    ...
    m_desc["add_ironing_caption"]       = _L("Left mouse button");
    m_desc["add_ironing"]               = _L("Add ironing");
    m_desc["remove_ironing_caption"]    = shift + _L("Left mouse button");
...
void GLGizmoIroning::show_tooltip_information(float caption_max, float x, float y)
{
        ...
        switch (m_tool_type) {
            case ToolType::BRUSH:
                if (m_cursor_type == TriangleSelector::POINTER) {
                    tip_items = {"add_ironing", "remove_ironing", "clipping_of_view"};
                } else {
                    tip_items = {"add_ironing", "remove_ironing", "cursor_size", "clipping_of_view"};
                }
                break;
            case ToolType::SMART_FILL:
                tip_items = {"add_ironing", "remove_ironing", "smart_fill_angle", "clipping_of_view"};
                break;
            default:
                break;
        }
...
void GLGizmoIroning::on_render_input_window(float x, float y, float bottom_limit)
{
    ...
    if (m_current_tool == ImGui::CircleButtonIcon || m_current_tool == ImGui::SphereButtonIcon) {
        ...
        std::string format_str = std::string("%.f") +
                                 I18N::translate_utf8("°", "Degree sign to use in the respective slider in ironing gizmo,"
