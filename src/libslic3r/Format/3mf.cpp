...
static constexpr const char* IRONING_FACETS_ATTR = "slic3rpe:ironing_facets";
...
ModelVolumeType type_from_string(const std::string &s)
{
    ...
    class _3MF_Importer : public _3MF_Base
    {
        ...
        struct Geometry
        {
            ...
            std::vector<std::string> ironing_facets;
...
//        model.adjust_min_z();
    ...
    bool _3MF_Importer::_handle_start_triangle(const char** attributes, unsigned int num_attributes)
    {
        ...
        m_curr_object.geometry.ironing_facets.push_back(get_attribute_value_string(attributes, num_attributes, IRONING_FACETS_ATTR));
        return true;
    }
    ...
    bool _3MF_Importer::_generate_volumes(ModelObject& object, const Geometry& geometry, const ObjectMetadata::VolumeMetadataList& volumes, ConfigSubstitutionContext& config_substitutions)
    {
        ...
        for (const ObjectMetadata::VolumeMetadata& volume_data : volumes) {
            ...
            // recreate custom supports, seam, mm segmentation, fuzzy skin and ironing from previously loaded attribute
            ...
            for (size_t i=0; i<triangles_count; ++i) {
                ...
                if (index < geometry.ironing_facets.size() && ! geometry.ironing_facets[index].empty())
                    volume->ironing_facets.set_triangle_from_string(i, geometry.ironing_facets[index]);
            }
        ...
        for (ModelVolume* volume : object.volumes) {
            ...
            for (int i = 0; i < int(its.indices.size()); ++ i) {
                ...
                if (!ironing_data_string.empty()) {
                    output_buffer += " ";
                    output_buffer += IRONING_FACETS_ATTR;
                    output_buffer += "=\"";
